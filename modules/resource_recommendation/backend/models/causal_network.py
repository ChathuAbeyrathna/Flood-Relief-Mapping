...
class CausalReliefNetwork:
    ...
    def create_target_variables(self, data):
        ...
        for new_col, (source_col, bins) in relief_mappings.items():
            if source_col in new_data.columns:
                ...
                new_data[new_col] = pd.cut(new_data[source_col], bins=bins, labels=labels, include_lowest=True)
        return new_data
    ...
    def discretize_data(self, df):
        ...
        if 'Affected_Population' in discretized_df.columns:
            ...
            discretized_df['Affected_Population'] = pd.cut(discretized_df['Affected_Population'], bins=bins, labels=labels, include_lowest=True)

        if 'Children_%' in discretized_df.columns:
            bins = [0, 0.15, 0.25, 0.35, 1.0]
            labels = ['Low', 'Medium', 'High', 'Very_High']
            discretized_df['Children_Percentage'] = pd.cut(discretized_df['Children_%'], bins=bins, labels=labels, include_lowest=True)

        if 'Elderly_%' in discretized_df.columns:
            bins = [0, 0.10, 0.15, 0.20, 1.0]
            labels = ['Low', 'Medium', 'High', 'Very_High']
            discretized_df['Elderly_Percentage'] = pd.cut(discretized_df['Elderly_%'], bins=bins, labels=labels, include_lowest=True)

        return discretized_df

    def build_network(self, training_data):
        print("\n" + "=" * 50)
        print("BUILDING CAUSAL NETWORK")
        print("=" * 50)
        ...
        print(f"Nodes: {available_nodes}")
        print(f"Edges: {available_edges}")
        print(f"Samples: {len(network_data)}")
        ...
        try:
            ...
            print("✅ Trained with Bayesian Estimator")
        except:
            self.model.fit(network_data, estimator=MaximumLikelihoodEstimator)
            print("✅ Trained with MLE")

        self.inference = VariableElimination(self.model)
        print(f"Network: {self.model.number_of_nodes()} nodes, {self.model.number_of_edges()} edges")
    ...
    def predict_relief_needs(self, affected_population, children_pct, elderly_pct, flood_severity='Medium'):
        ...
        # Categorize
        if affected_population < 5000: pop_cat = '0-5k'
        elif affected_population < 10000: pop_cat = '5k-10k'
        elif affected_population < 20000: pop_cat = '10k-20k'
        elif affected_population < 30000: pop_cat = '20k-30k'
        elif affected_population < 50000: pop_cat = '30k-50k'
        else: pop_cat = '50k+'

        children_cat = 'High' if children_pct > 0.25 else 'Medium'
        elderly_cat = 'High' if elderly_pct > 0.15 else 'Medium'

        evidence = {}
        if 'Affected_Population' in self.model.nodes(): evidence['Affected_Population'] = pop_cat
        if 'Flood_Severity' in self.model.nodes(): evidence['Flood_Severity'] = flood_severity
        if 'Children_Percentage' in self.model.nodes(): evidence['Children_Percentage'] = children_cat
        if 'Elderly_Percentage' in self.model.nodes(): evidence['Elderly_Percentage'] = elderly_cat
    ...
    def get_explainable_recommendation(self, input_data, predictions):
        ...
        explanation['reasoning_steps'].append(
            f"📍 {input_data['affected_population']:,} people, {input_data['children_pct']*100:.1f}% children, {input_data['elderly_pct']*100:.1f}% elderly."
        )

        if input_data['flood_severity'] == 'High':
            explanation['reasoning_steps'].append("⚠️ HIGH severity - substantial relief required!")
        ...
        if predictions:
            for var, pred in predictions.items():
                if var == 'Water_Need':
                    explanation['reasoning_steps'].append(f"💧 Water: {pred['most_likely']} priority")
                    if pred['most_likely'] in ['High', 'Very_High']:
                        explanation['recommendations'].append("Deploy clean water (3L/person/day)")
                    ...
                    explanation['reasoning_steps'].append(f"🩸 Sanitary: {pred['most_likely']} priority")
                    if pred['most_likely'] in ['High', 'Very_High']:
                        explanation['recommendations'].append("Deploy sanitary pads")
                elif var == 'Hygiene_Need':
                    explanation['reasoning_steps'].append(f"🧼 Hygiene: {pred['most_likely']} priority")
                    if pred['most_likely'] in ['High', 'Very_High']:
                        explanation['recommendations'].append("Distribute soap, toothpaste, toothbrushes")

        explanation['summary'] = f"Priority: {explanation['priority_level']} level relief needed."
        return explanation