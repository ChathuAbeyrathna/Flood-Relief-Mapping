...
def run_layer4_validation_pipeline():
    ...
    for test_year in validation_years:
        ...
        std_bart = SklearnModel(n_trees=50, n_samples=200, n_burn=200, n_chains=4, sigma_a=2.0, sigma_b=0.75, n_jobs=1)
        ...
        # ------------------------------------------------------------------
        model = HeteroscedasticBART(n_trees_mean=50, n_samples=200, n_burn=200, n_chains=4, n_trees_var=200, max_depth_var=4)
    ...
    print("=" * 85)
    cv_df.to_csv(os.path.join(output_dir, "layer4_novelty_comparison_report.csv"), index=False)
